...
/**
 * Retourne un payload de facture pré-rempli à partir du rapport de l'intervention,
 * SANS la persister. Permet à l'UI d'afficher un éditeur live avec preview.
 *
 * GET /api/interventions/[id]/facture-preview
 *   → { prefill: RapportToFacturePrefill }
 */
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  const { data: interv, error: intErr } = await sb
    .from('interventions')
    .select('id, reference, client_id, technicien_id, agence, type_intervention, adresse_chantier, ville, code_postal, date_realisee, date_prevue, rapport_json')
